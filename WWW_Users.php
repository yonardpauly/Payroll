...
$ModuleList = array('Orders', 'Receivables', 'Payables', 'Purchasing', 'Inventory', 'Manufacturing', 'General Ledger', 'Payroll', 'Setup');
...
if (isset($_POST['submit'])) {
	...
	if ( strlen($_POST['UserID']) < 4 ){
		$InputError = 1;
		echo 'The user ID entered must be at least 4 characters long', 'error';
...
/*SelectedUser could also exist if submit had not been clicked this code would not run in this case cos submit is false of course  see the delete code below*/
		...
		$Access = 8;
		$sql = "UPDATE www_users SET realname='" . DB_escape_string($db, $_POST['RealName']) . "',
						customerid='" . DB_escape_string($db, $_POST['Cust']) ."',
						phone='" . DB_escape_string($db, $_POST['Phone']) ."',
						email='" . DB_escape_string($db, $_POST['Email']) ."',
						".$UpdatePassword."
						branchcode='" . DB_escape_string($db, $_POST['BranchCode']) . "',
		...
		$sql = "INSERT INTO www_users (userid,
						...
						language)
					VALUES ('" . $_POST['UserID'] . "',
						'" . DB_escape_string($db, $_POST['RealName']) ."',
						'" . CryptPass($_POST['Password']) ."',
						'" . DB_escape_string($db, $_POST['Phone']) . "',
						'" . DB_escape_string($db, $_POST['Email']) ."',
						'" . $_POST['PageSize'] ."',
						'$Access',
						'" . $ModulesAllowed . "',
