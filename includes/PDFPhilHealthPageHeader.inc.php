<?php
	...
	$FontSize =10;
	$pdf->selectFont('./fonts/Helvetica-Bold.afm');
	$Heading = _('PhilHealth Monthly Remittance for All Employees');
	...
	$pdf->addText($Page_Width-$Right_Margin-120,$YPos,$FontSize,
		...
		. '   '. _('Page'). ' ' . $PageNumber);
	$YPos -= (1 * $line_height);	
	$Heading1 = _('For the month of ') . $PhilHealthMonthStr .', ' .$PhilHealthYear;
	$pdf->addText($Left_Margin,$YPos,$FontSize,$Heading1);

	$YPos -= (2 * $line_height);
	$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,150,$FontSize,'Full Name');
	$LeftOvers = $pdf->addTextWrap($Left_Margin+200,$YPos,50,$FontSize,'PhilHealth Number','right');
